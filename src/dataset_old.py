...
#         # Calculate inverse relation ID for tail sub-graph exclusion: (t -[r+num_rel]-> h)
...
def _collate_entity(batch: List[int], neighbor_index: NeighborIndex) -> Dict[str, Any]:
    ...
    return {
        "entity": torch.tensor(batch, dtype=torch.long).unsqueeze(1),
        "neighbor": _pack_edges(ei_list, ea_list),
    }
